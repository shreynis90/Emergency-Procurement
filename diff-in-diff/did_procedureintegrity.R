...
t.test(d_oneyearpost$procedureintegrity)
t.test(d_oneyearpre$procedureintegrity)

##Two year ----
d1_twoyear <- disaster1_final %>% filter(contractyear == 2007|contractyear == 2011)%>% filter(loc == 1)
d2_twoyear <- disaster2_final %>% filter(contractyear == 2010|contractyear == 2013|contractyear == 2014)%>% filter(loc == 1)
...
t.test(d_twoyearpost$procedureintegrity)
t.test(d_twoyearpre$procedureintegrity)
...
t.test(d_threeyearpost$procedureintegrity)
t.test(d_threeyearpre$procedureintegrity)

##
year <- c("t =-3", "t =-2", "t =-1","t = 1","t = 2","t = 3")
meanprocedureintegrity <- c(t.test(d_threeyearpre$procedureintegrity)$'estimate', t.test(d_twoyearpre$procedureintegrity)$'estimate',t.test(d_oneyearpre$procedureintegrity)$'estimate',t.test(d_oneyearpost$procedureintegrity)$'estimate',t.test(d_twoyearpost$procedureintegrity)$'estimate',t.test(d_threeyearpost$procedureintegrity)$'estimate')

lowerbound <- c(t.test(d_threeyearpre$procedureintegrity)$'conf.int'[1],t.test(d_twoyearpre$procedureintegrity)$'conf.int'[1],t.test(d_oneyearpre$procedureintegrity)$'conf.int'[1],t.test(d_oneyearpost$procedureintegrity)$'conf.int'[1],t.test(d_twoyearpost$procedureintegrity)$'conf.int'[1],t.test(d_threeyearpost$procedureintegrity)$'conf.int'[1])

upperbound <- c(t.test(d_threeyearpre$procedureintegrity)$'conf.int'[2],t.test(d_twoyearpre$procedureintegrity)$'conf.int'[2],t.test(d_oneyearpre$procedureintegrity)$'conf.int'[2],t.test(d_oneyearpost$procedureintegrity)$'conf.int'[2],t.test(d_twoyearpost$procedureintegrity)$'conf.int'[2],t.test(d_threeyearpost$procedureintegrity)$'conf.int'[2])

df <- data.frame(year, meanprocedureintegrity, lowerbound,upperbound)

df$year <- factor(df$year, levels = df$year)
p <- ggplot(df, aes(year, as.numeric(as.character(meanprocedureintegrity)))) +        # ggplot2 plot with confidence intervals
  ...
  annotate(geom = "text",
           ...
           y = 0.2,
           angle = 90, 
           vjust = 26) + ylab("Non-Open Procedure")
