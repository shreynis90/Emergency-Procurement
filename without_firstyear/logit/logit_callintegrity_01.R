...
model_logit<- glm(callintegrity ~ treatmentstatus + factor(newcpv) + buyer+ log_contractvalue + contractyear + contractmonth, family ="binomial", data = italy_reg_filtered)

saveRDS(model_logit, 'tables/robustness/table_21/2_logit_callintegrity_01_model.rds')

n_after = sum(italy_reg_filtered$treatmentstatus[italy_reg_filtered$treatmentstatus == 1])

saveRDS(n_after, 'tables/robustness/table_21/2_logit_callintegrity_01_n_after.rds')

# summary.glm(model_logit)

saveRDS(RsqGLM(model_logit), 'tables/robustness/table_21/2_logit_callintegrity_01_se.rds')

# summary(margins(model_logit))

m <- margins::margins(model_logit, variables = c('treatmentstatus'), type = "response") %>% 
  broom::tidy() %>% 
  mutate(term = 'No tender call')
saveRDS(m, 'tables/robustness/table_21/2_logit_callintegrity_01.rds')
