...
##Removing Missings 
italy<- italy %>% filter(!is.na(tender_indicator_INTEGRITY_SINGLE_BID))

##Declaring a new Single bid integrity variable
italy$singlebidintegrity <- ifelse(italy$tender_indicator_INTEGRITY_SINGLE_BID == 0, 1, 0)
...
##Number of Treated contracts by single bid integrity variable
ncontracts1 <- italy_disaster1 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, singlebidintegrity)%>% mutate(numberofcontracts = n()) %>% select(Date, numberofcontracts, treatcon, disnumber, singlebidintegrity) %>% distinct()
...
ncontracts2 <- italy_disaster2 %>% filter(treatcon == 1) %>% group_by(Date, singlebidintegrity)%>% mutate(numberofcontracts = n()) %>% select(Date, numberofcontracts, treatcon, disnumber, singlebidintegrity) %>% distinct()
...
ncontracts3 <- italy_disaster3 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, singlebidintegrity)%>% mutate(numberofcontracts = n()) %>% select(Date, numberofcontracts, treatcon, disnumber, singlebidintegrity) %>% distinct()
...
ncontracts4 <- italy_disaster4 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, singlebidintegrity)%>% mutate(numberofcontracts = n()) %>% select(Date, numberofcontracts, treatcon, disnumber, singlebidintegrity) %>% distinct()
...
ncontracts5 <- italy_disaster5 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, singlebidintegrity)%>% mutate(numberofcontracts = n()) %>% select(Date, numberofcontracts, treatcon, disnumber, singlebidintegrity) %>% distinct()
...
ncontracts5$ord<- NA
ncontracts5$time<- NA
##Preparing to pile disasters together into one analysis----
...
##MONTHLY ANALYSIS (1 year before and after) ----

ncontracts<- rbind(ncontracts1, ncontracts2, ncontracts3, ncontracts4, ncontracts5)

ncontracts <- ncontracts %>% group_by(time, singlebidintegrity)%>% mutate(numberofcontracts = sum(numberofcontracts)) %>% select(time, numberofcontracts, treatcon, ord, singlebidintegrity) %>% distinct()
...
ggplot(data=ncontracts, aes(x=fct_inorder(time), y=numberofcontracts, fill=as.factor(singlebidintegrity))) + geom_bar(stat="identity")+  scale_fill_brewer(palette="Paired")+
  theme_minimal()+   theme(axis.text.x=element_text(angle=90,hjust=1)) +  geom_vline(xintercept=ncontracts$ord[11] ,linetype=1, colour="black")+ labs(title="Monthly Number of Contracts by Single Bid Integrity", x="Dates (unit in Months)", y = "Monthly number of contracts" , subtitle="T = 0 depicts disaster incidence", fill = "1: Single Bidder")


##QUARTERLY ANALYSIS (3 years before and after)----
ncontracts<- NULL
ncontracts<- rbind(ncontracts1, ncontracts2, ncontracts3, ncontracts4, ncontracts5)
ncontracts<- as.data.frame(ncontracts)
ncontracts$ord<- as.numeric(as.character(ncontracts$ord))
...
ncontracts <- ncontracts %>% group_by(time, singlebidintegrity)%>% mutate(numberofcontracts = sum(numberofcontracts)) %>% select(singlebidintegrity,time, numberofcontracts, treatcon, ord) %>% distinct()

ncontracts <- ncontracts%>% filter(ord >= -36 & ord<= 36)

##Generating blanks for those time periods that have no contracts
...
for (j in 1:nrow(k)) {
  ...
  if(k$Freq[j] == 1)
  {
    ...
    a<- rbind(a, c(singlebidintegrity = 0, time = t, numberofcontracts = 0,treatcon =1, ord=k$Var1[j]))
  }
}
a<- as.data.frame(a)
a$singlebidintegrity<- as.numeric(as.character(a$singlebidintegrity))
...
ncontracts <- ncontracts %>% group_by(date, singlebidintegrity)%>% mutate(numberofcontracts = round(sum(numberofcontracts),2)) %>% select(singlebidintegrity,date, numberofcontracts, treatcon) %>% distinct()

ncontracts <- ncontracts %>% group_by(date)%>% mutate(share = round(numberofcontracts/sum(numberofcontracts),2)) %>% select(singlebidintegrity,date, numberofcontracts, treatcon, share) %>% distinct()

ggplot(data=ncontracts, aes(x=fct_inorder(date), y=numberofcontracts, fill=as.factor(singlebidintegrity))) + geom_bar(stat="identity")+  scale_fill_brewer(palette="Dark2")+
  theme_minimal()+   theme(axis.text.x=element_text(angle=90,hjust=1)) +  geom_vline(xintercept=ncontracts$ord[25] ,linetype=1, colour="black")+ labs(title="Quarterly Number of Contracts By Single Bidder Integrity", x="Dates (unit in Quarters)", y = "Quarterly Number of Contracts" , subtitle="Disaster Month depicts the share of contracts in the Disaster Month", fill = "1: Single Bidder")

ggplot(data=ncontracts, aes(x=fct_inorder(date), y=share, fill=as.factor(singlebidintegrity))) + geom_bar(stat="identity")+  scale_fill_brewer(palette="Dark2")+
  theme_minimal()+   theme(axis.text.x=element_text(angle=90,hjust=1)) +  geom_vline(xintercept=ncontracts$ord[25] ,linetype=1, colour="black")+ labs(title="Quarterly Share of Contracts By Single Bidder Integrity", x="Dates (unit in Quarters)", y = "Quarterly Share of Contracts" , subtitle="Disaster Month depicts the share of contracts in the Disaster Month", fill = "1: Single Bidder")


##T-tests (in order 3years, 2years, 1 year windows)----

ncontracts_before_3 <- ncontracts[c(1:24),]
ncontracts_after_3 <- ncontracts[c(27:50),]
ncontracts_before_3.gr1 <- ncontracts_before_3 %>% filter(singlebidintegrity == 1)
ncontracts_after_3.gr1 <- ncontracts_after_3 %>% filter(singlebidintegrity == 1)
...
ncontracts_before_2.gr1 <- ncontracts_before_2 %>% filter(singlebidintegrity == 1)
ncontracts_after_2.gr1 <- ncontracts_after_2 %>% filter(singlebidintegrity == 1)
...
model3_logit_small<- glm(singlebidintegrity ~ treatmentstatus + factor(newcpv) + buyer + log_contractvalue + contractyear + contractmonth, family ="binomial", data = italy_reg3)
summary.glm(model3_logit_small)
RsqGLM(model3_logit_small)


model3_ols_small<- lm(singlebidintegrity ~ treatmentstatus + contractmonth + contractyear + factor(newcpv) + log_contractvalue, data = italy_reg3)
...
model2_logit_small<- glm(singlebidintegrity ~ treatmentstatus + factor(newcpv) + log_contractvalue + buyer + contractyear + contractmonth, family ="binomial", data = italy_reg2)
summary.glm(model2_logit_small)
RsqGLM(model2_logit_small)

model2_ols_small<- lm(singlebidintegrity ~ treatmentstatus + contractmonth + contractyear + factor(newcpv) + log_contractvalue + buyer, data = italy_reg2)
...
model1_logit_small<- glm(singlebidintegrity ~ treatmentstatus + contractmonth + contractyear + factor(newcpv) + log_contractvalue + buyer, family ="binomial", data = italy_reg1)
summary.glm(model1_logit_small)
RsqGLM(model1_logit_small)

model1_ols_small<- lm(singlebidintegrity ~ treatmentstatus + contractmonth + contractyear + factor(newcpv) + log_contractvalue + buyer, data = italy_reg1)
